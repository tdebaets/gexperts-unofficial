...
const
    ...
    DEFAULT_TITLEBAR_BUTTONS_SPACING = 0;
...
type
    ...
    TEIPanel = class(TCustomPanel)
      private
        ...
        function CalcSplitNewSize(DoEnd: BOOLEAN): Integer;
      protected
      ...
      published
        ...
        property LiveSplit: BOOLEAN read FLiveSplit write FLiveSplit;
        end;
    ...
    TSplitterControl = class
      ...
      public
        ...
        procedure BeginSizing(Vertical: BOOLEAN; DMIN: Integer; DMax: Integer; Form: TCustomForm; DrawLine: BOOLEAN);
    ...
    if (ButtonVisible) then begin
        ...
        case FTitleBarAlign of
            talLeft, talRight: begin
                ...
                NextButtonPos := NextButtonPos + Button.Height + DEFAULT_TITLEBAR_BUTTONS_SPACING;
            ...
            talTop, talBottom: begin
                ...
                NextButtonPos := NextButtonPos - Button.Width - DEFAULT_TITLEBAR_BUTTONS_SPACING;
                end; {talTop, talBottom}
            end; {case}
        end; {if (ButtonVisible) then begin}
end;
...
begin
    ...
    if ((FTitleBar <> nil) and FTitleBar.Visible) then begin
        case FTitleBarAlign of
            ...
            talTop, talBottom: begin
                FTitleBar.Height := DEFAULT_TITLEBAR_WIDTH;
...
//                NextButtonPos := FTitleBar.Width - DEFAULT_TITLEBAR_BUTTONS_BORDER - DEFAULT_TITLEBAR_BUTTONS_WIDTH;
                NextButtonPos := Width - DEFAULT_TITLEBAR_BUTTONS_BORDER - DEFAULT_TITLEBAR_BUTTONS_WIDTH - FTitleBar.BevelWidth;
                end; {talTop, talBottom}
        end; {case}

//    debuglist: TStringlist = nil;
        ...
        if (FMinMaxButton <> nil) then begin
...
begin
    ...
    if (form <> nil) then begin
        if (Button = mbLeft) and (Shift = [ssLeft]) then begin
            ...
            FSplitterControl.BeginSizing(vertical, dmin, dmax, Form, not FLiveSplit);
            end;
        end;
end;

function TEIPanel.CalcSplitNewSize(DoEnd: BOOLEAN): Integer;
var minsize:Integer;
begin
    if TitleBarVisible then
    ...
    if (DoEnd) then begin
        case Align of
            alLeft:     result := FSplitterControl.EndSizing          + FSplitterPanel.Width;
            alRight:    result := Width - FSplitterControl.EndSizing  + FSplitterPanel.Width;
            alTop:      result := FSplitterControl.EndSizing          + FSplitterPanel.Height;
            alBottom:   result := Height - FSplitterControl.EndSizing + FSplitterPanel.Height;
            end;
        end
    else begin
        case Align of
            alLeft:     result := FSplitterControl.CurrentSize          + FSplitterPanel.Width;
            alRight:    result := Width - FSplitterControl.CurrentSize  + FSplitterPanel.Width;
            alTop:      result := FSplitterControl.CurrentSize          + FSplitterPanel.Height;
            alBottom:   result := Height - FSplitterControl.CurrentSize + FSplitterPanel.Height;
            end;
        end;

    if (result < minsize) then
        result := minsize;
    if (FLiveSplit) then begin
        if (result > FSplitterControl.Max) then
            result := FSplitterControl.Max;
        end;
end;
...
begin
    if FSplitterControl.Sizing then begin
        ...
        if (FLiveSplit) then begin
            newSize := CalcSplitNewSize(FALSE);
            if (newSize < FSplitterControl.Min) then
                newSize := FSplitterControl.Min;
            if (newSize > FSplitterControl.Max) then
                newSize := FSplitterControl.Max;
                
            case Align of
                alLeft,alRight:     Width := newsize;
                alTop, alBottom:    Height := newsize;
                end;
            end;
        end;
end;
...
{
procedure TEIPanel.WMSize(var msg: TMessage);
begin
    inherited;
...
begin
    ...
    if ((result<0) and (not IsTitleBarVertical)) or ((result>FTitleBar.Height) and (IsTitleBarVertical)) then
        result:=0;
end;
...
procedure TSplitterControl.BeginSizing(Vertical: BOOLEAN; DMIN: Integer; DMax: Integer; Form: TCustomForm; DrawLine: BOOLEAN);
begin
    FForm := Form;
    FMin := Dmin;
    FMax := DMax;
...
function TSplitterControl.GetCurrentSize: Integer;
var P: TPoint;
begin
